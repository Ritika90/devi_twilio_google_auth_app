class RegistrationsController < Devise::RegistrationsController
  ...
  def create
    ...
    if @user.status == "message"
      ...
      message = client.messages.create from: '+19136672012', to: "#{resource.phone}", body: "Hello there, your OTP is #{resource.otp_code}"
      resource.confirmation_sent_at = Time.now
      resource.save
      session[:code] =  resource.otp_code
      session[:user] =  resource.id
      redirect_to twilio_otp_path
    else
